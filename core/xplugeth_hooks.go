...
func pluginNewHead(block *types.Block, hash common.Hash, logs []*types.Log) {
	td, err := utils.GetTd(hash)
	if err != nil {
		log.Error("error returned from call to GetTD, pluginNewHead", "err", err)
		td = big.NewInt(0)
	}
...
func pluginReorg(commonBlock *types.Header, oldChain, newChain []*types.Header) {
